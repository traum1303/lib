...
@section('content')
    <div class="max-w-3xl mx-auto px-4 py-8">
        ...
        <form method="POST" action="{{ route('authors.update', $author) }}" class="bg-white border border-gray-200 rounded-lg shadow-sm">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            name="second_name"
                            id="second_name"
                            value="{{old('second_name', $author->second_name)}}"
                    ...
                    <div>
                        ...
                        <select id="gender" name="gender" class="block border-1 w-full p-3 rounded-md border-gray-300 shadow-sm text-sm font-mono
                            ...
                            @foreach (\App\Enums\Gender::cases() as $gender)
                                <option value="{{ $gender->value }}" @selected(\App\Enums\Gender::tryFrom(old('gender', $author->gender->value)) === $gender)>
                                    {{ $gender->label() }}
                                </option>
                            @endforeach
                              </select>
